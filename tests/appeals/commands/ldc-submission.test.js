...
const exampleLDCSubmissionSchema = {
	casedata: {
...
describe('LDC submission command schema', () => {
	...
	it('should allow valid schema for LDC', () => {
		const validationResult = ajv.validate(schema, exampleLDCSubmissionSchema);
		assert.strictEqual(validationResult, true);
	});

	it('should enforce LDC specific props', () => {
		const test = structuredClone(exampleLDCSubmissionSchema);
		delete test.casedata.siteUseAtTimeOfApplication;
		delete test.casedata.applicationMadeUnderActSection;
