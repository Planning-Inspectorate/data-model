# generated by datamodel-codegen:
#   filename:  appeal-s78.schema.json

from __future__ import annotations

from enum import Enum, StrEnum
from uuid import UUID

from pydantic import AwareDatetime, BaseModel, ConfigDict, Field, RootModel, constr


class CaseStatus(StrEnum):
    """
    The processing status for the appeal
    """

    assign_case_officer = "assign_case_officer"
    validation = "validation"
    ready_to_start = "ready_to_start"
    lpa_questionnaire = "lpa_questionnaire"
    issue_determination = "issue_determination"
    statements = "statements"
    evidence = "evidence"
    witnesses = "witnesses"
    final_comments = "final_comments"
    complete = "complete"
    invalid = "invalid"
    closed = "closed"
    withdrawn = "withdrawn"
    awaiting_transfer = "awaiting_transfer"
    transferred = "transferred"
    event = "event"
    awaiting_event = "awaiting_event"


class CaseType(StrEnum):
    """
    The internal code for an appeal type, e.g. D (Householder)
    """

    C = "C"
    D = "D"
    F = "F"
    G = "G"
    H = "H"
    L = "L"
    Q = "Q"
    S = "S"
    V = "V"
    W = "W"
    X = "X"
    Y = "Y"
    Z = "Z"
    ZA = "ZA"
    ZP = "ZP"


class CaseProcedure(Enum):
    """
    The type of procedure for the appeal
    """

    written = "written"
    hearing = "hearing"
    inquiry = "inquiry"
    writtenPart1 = "writtenPart1"
    writtenPart2 = "writtenPart2"
    NoneType_None = None


class AllocationLevel(Enum):
    """
    A level used for allocation purposes
    """

    A = "A"
    B = "B"
    C = "C"
    D = "D"
    E = "E"
    F = "F"
    G = "G"
    H = "H"
    NoneType_None = None


class AllocationBand(Enum):
    """
    A band used for allocation purposes
    """

    int_1 = 1
    int_2 = 2
    int_3 = 3
    NoneType_None = None


class CaseValidationOutcome(Enum):
    """
    The outcome of the validation action
    """

    valid = "valid"
    invalid = "invalid"
    incomplete = "incomplete"
    NoneType_None = None


class LinkedCaseStatus(Enum):
    """
    Indicates if the case is linked, and the type of relationship
    """

    lead = "lead"
    child = "child"
    NoneType_None = None


class LpaQuestionnaireValidationOutcome(Enum):
    """
    The outcome of the validation action
    """

    complete = "complete"
    incomplete = "incomplete"
    NoneType_None = None


class CaseDecisionOutcome(Enum):
    """
    The final outcome for the decision
    """

    allowed = "allowed"
    split_decision = "split_decision"
    dismissed = "dismissed"
    invalid = "invalid"
    notice_upheld = "notice_upheld"
    planning_permission_granted = "planning_permission_granted"
    quashed_on_legal_grounds = "quashed_on_legal_grounds"
    notice_varied_and_upheld = "notice_varied_and_upheld"
    NoneType_None = None


class ApplicationDecision(Enum):
    """
    The outcome of the original LPA decision
    """

    granted = "granted"
    refused = "refused"
    not_received = "not_received"
    NoneType_None = None


class KnowsOtherOwners(Enum):
    """
    Indicates if the appellant knows other owners of the site
    """

    Yes = "Yes"
    No = "No"
    Some = "Some"
    NoneType_None = None


class KnowsAllOwners(Enum):
    """
    Indicates if the appellant knows all owners of the site
    """

    Yes = "Yes"
    No = "No"
    Some = "Some"
    NoneType_None = None


class NotificationMethodEnum(StrEnum):
    notice = "notice"
    letter = "letter"
    advert = "advert"
    not_notified = "not_notified"


class NeighbouringSiteAddress(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    neighbouringSiteAddressLine1: str = Field(..., examples=["96 The Avenue"])
    """
    First line of address of the site
    """
    neighbouringSiteAddressLine2: str | None
    """
    Second line of address of the site
    """
    neighbouringSiteAddressTown: str = Field(..., examples=["Maidstone"])
    """
    Town / City of the site address
    """
    neighbouringSiteAddressCounty: str | None = Field(..., examples=["Kent"])
    """
    County of the site address
    """
    neighbouringSiteAddressPostcode: str = Field(..., examples=["MD21 5XY"])
    """
    Postal code of the site address
    """
    neighbouringSiteAccessDetails: str | None = Field(
        ..., examples=["Ongoing road works at this address"]
    )
    """
    Provided information on site accessibility on this address
    """
    neighbouringSiteSafetyDetails: str | None = Field(
        ..., examples=["Ongoing road works at this address"]
    )
    """
    Provided information on site health and safety on this address
    """


class AppellantProcedurePreference(Enum):
    """
    The procedure preference indicated by the appellant
    """

    written = "written"
    hearing = "hearing"
    inquiry = "inquiry"
    NoneType_None = None


class EiaEnvironmentalImpactSchedule(Enum):
    """
    The impact schedule from EIA
    """

    schedule_1 = "schedule-1"
    schedule_2 = "schedule-2"
    NoneType_None = None


class EiaDevelopmentDescription(Enum):
    """
    The development description from EIA
    """

    agriculture_aquaculture = "agriculture-aquaculture"
    change_extensions = "change-extensions"
    chemical_industry = "chemical-industry"
    energy_industry = "energy-industry"
    extractive_industry = "extractive-industry"
    food_industry = "food-industry"
    infrastructure_projects = "infrastructure-projects"
    mineral_industry = "mineral-industry"
    other_projects = "other-projects"
    production_processing_of_metals = "production-processing-of-metals"
    rubber_industry = "rubber-industry"
    textile_industries = "textile-industries"
    tourism_leisure = "tourism-leisure"
    NoneType_None = None


class LpaProcedurePreference(Enum):
    """
    The procedure preference indicated by the LPA
    """

    written = "written"
    hearing = "hearing"
    inquiry = "inquiry"
    NoneType_None = None


class TypeOfPlanningApplication(Enum):
    full_appeal = "full-appeal"
    householder_planning = "householder-planning"
    outline_planning = "outline-planning"
    prior_approval = "prior-approval"
    reserved_matters = "reserved-matters"
    removal_or_variation_of_conditions = "removal-or-variation-of-conditions"
    listed_building = "listed-building"
    minor_commercial_development = "minor-commercial-development"
    advertisement = "advertisement"
    lawful_development_certificate = "lawful-development-certificate"
    discontinuance_notice = "discontinuance-notice"
    permission_in_principle = "permission-in-principle"
    NoneType_None = None


class DevelopmentType(Enum):
    householder = "householder"
    change_of_use = "change-of-use"
    major_dwellings = "major-dwellings"
    major_industry_storage = "major-industry-storage"
    major_offices = "major-offices"
    major_retail_services = "major-retail-services"
    major_traveller_caravan = "major-traveller-caravan"
    mineral_workings = "mineral-workings"
    minor_dwellings = "minor-dwellings"
    minor_industry_storage = "minor-industry-storage"
    minor_offices = "minor-offices"
    minor_retail_services = "minor-retail-services"
    minor_traveller_caravan = "minor-traveller-caravan"
    other_major = "other-major"
    other_minor = "other-minor"
    NoneType_None = None


class AdvertType(Enum):
    field_4_6_Sheet_Non_Illuminated = "4-6 Sheet Non-Illuminated"
    field_4_6_Sheet_Illuminated = "4-6 Sheet Illuminated"
    field_12_16_32_Sheet_Non_Illuminated = "12/16/32 Sheet Non-Illuminated"
    field_12_16_32_Sheet_Illuminated = "12/16/32 Sheet Illuminated"
    field_48_64_96_Sheet_Non_Illuminated = "48/64/96 Sheet Non-Illuminated"
    field_48_64_96_Sheet_Illuminated = "48/64/96 Sheet Illuminated"
    Fascia_Projecting_Signs_Non_Illuminated = "Fascia Projecting Signs Non-Illuminated"
    Fascia_Projecting_Signs_Illuminated = "Fascia Projecting Signs Illuminated"
    Directional___Others_Non_Illuminated = "Directional & Others Non-Illuminated"
    Directional___Others_Illuminated = "Directional & Others Illuminated"
    Monolith_Totem_Signs_Non_Illuminated = "Monolith/Totem Signs Non-Illuminated"
    Monolith_Totem_Signs_Illuminated = "Monolith/Totem Signs Illuminated"
    Other = "Other"
    NoneType_None = None


class AdvertDetail(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    advertType: AdvertType | None
    isAdvertInPosition: bool | None
    """
    Is the advertisement in position?
    """
    isSiteOnHighwayLand: bool | None
    """
    Is the site on highway land?
    """


class AppealGroundLetter(Enum):
    a = "a"
    b = "b"
    c = "c"
    d = "d"
    e = "e"
    f = "f"
    g = "g"
    NoneType_None = None


class EnforcementAppealGroundsDetail(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    appealGroundLetter: AppealGroundLetter | None = None
    groundForAppealStartDate: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    groundFacts: str | None = None
    """
    Facts supporting the appeal ground
    """


class AppealGroundLetter1(Enum):
    a = "a"
    b = "b"
    c = "c"
    d = "d"
    e = "e"
    f = "f"
    g = "g"
    h = "h"
    i = "i"
    j = "j"
    k = "k"
    NoneType_None = None


class ApplicationElbAppealGroundsDetail(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    appealGroundLetter: AppealGroundLetter1 | None = None
    groundForAppealStartDate: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    groundFacts: str | None = None
    """
    Facts supporting the appeal ground
    """


class ApplicationPartOrWholeDevelopment(Enum):
    """
    Did the application relate to all or part of the development
    """

    all_of_the_development = "all-of-the-development"
    part_of_the_development = "part-of-the-development"
    NoneType_None = None


class ApplicationMadeUnderActSection(Enum):
    existing_development = "existing-development"
    proposed_use_of_a_development = "proposed-use-of-a-development"
    proposed_changes_to_a_listed_building = "proposed-changes-to-a-listed-building"
    NoneType_None = None


class AppealUnderActSection(Enum):
    existing_development = "existing-development"
    proposed_use_of_a_development = "proposed-use-of-a-development"
    proposed_changes_to_a_listed_building = "proposed-changes-to-a-listed-building"
    NoneType_None = None


class DiscontinuanceNoticeServedReason(Enum):
    risk_of_injury_to_the_amenity_of_locality = (
        "risk-of-injury-to-the-amenity-of-locality"
    )
    danger_to_members_of_the_public = "danger-to-members-of-the-public"
    other = "other"
    NoneType_None = None


class Value(Enum):
    adopted_a_new_local_plan = "adopted-a-new-local-plan"
    national_policy_change = "national-policy-change"
    court_judgement = "court-judgement"
    other = "other"
    NoneType_None = None


class SignificantChangesAffectingApplicationAppellantItem(BaseModel):
    value: Value | None
    comment: str | None = None


class SignificantChangesAffectingApplicationLpaItem(BaseModel):
    value: Value | None
    comment: str | None = None


class AppealS78Case1(BaseModel):
    """
    Group B - Schema defining the metadata for an appeal
    """

    model_config = ConfigDict(
        extra="allow",
    )
    caseId: int | None = Field(..., examples=[1200])
    """
    Internal case identifier
    """
    caseReference: str = Field(..., examples=["7002471"])
    """
    External case identifier
    """
    submissionId: UUID | None = Field(
        None, examples=["7f45748c-2342-4b59-b934-ad6183e8d834"]
    )
    """
    Case submission id - link to draft submission created by appellant
    """
    caseStatus: CaseStatus
    """
    The processing status for the appeal
    """
    caseType: CaseType
    """
    The internal code for an appeal type, e.g. D (Householder)
    """
    caseProcedure: CaseProcedure | None
    """
    The type of procedure for the appeal
    """
    lpaCode: str = Field(..., examples=["Q9999"])
    """
    A unique identifier for the Local Planning Authority
    """
    caseOfficerId: str | None
    """
    Unique identifier for the case officer assigned to the case
    """
    inspectorId: str | None
    """
    Unique identifier for the inspector assigned to the case
    """
    allocationLevel: AllocationLevel | None
    """
    A level used for allocation purposes
    """
    allocationBand: AllocationBand | None
    """
    A band used for allocation purposes
    """
    caseSpecialisms: list[str] | None = Field(
        ..., examples=[["Access", "Listed building and enforcement"], ["Schedule 1"]]
    )
    """
    A list of specialisms for allocation purposes
    """
    caseSubmittedDate: AwareDatetime = Field(..., examples=["2023-07-27T20:30:00.000Z"])
    """
    The date the appeal was submitted by the appellant
    """
    caseCreatedDate: AwareDatetime = Field(..., examples=["2023-07-27T20:30:00.000Z"])
    """
    The date the appeal was received
    """
    caseUpdatedDate: AwareDatetime = Field(..., examples=["2023-07-27T20:30:00.000Z"])
    """
    The date the appeal was last updated in the back-office
    """
    caseValidDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date since when the appeal was considered valid
    """
    caseValidationDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date the appeal was validated in the back-office
    """
    caseValidationOutcome: CaseValidationOutcome | None
    """
    The outcome of the validation action
    """
    caseValidationInvalidDetails: list[str] | None
    """
    A list of reasons why the appeal is invalid
    """
    caseValidationIncompleteDetails: list[str] | None
    """
    A list of reasons why the appeal is incomplete
    """
    caseExtensionDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    When the validation outcome is incomplete, an extension may be granted to provide missing information
    """
    caseStartedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    A date indicating when the case was started, resulting in the creation of a timetable
    """
    casePublishedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    A date indicating when the case was published
    """
    linkedCaseStatus: LinkedCaseStatus | None
    """
    Indicates if the case is linked, and the type of relationship
    """
    leadCaseReference: str | None = Field(..., examples=["7002471"])
    """
    The reference of the lead case, if the case is a child case
    """
    lpaQuestionnaireDueDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    If the case is started and has a timetable, a deadline for the LPA to provide a response
    """
    lpaQuestionnaireSubmittedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date the LPA provided a response to the case
    """
    lpaQuestionnaireCreatedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date the LPA response was receeived
    """
    lpaQuestionnairePublishedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date indicating when the questionnaire review was completed and the questionnaire published
    """
    lpaQuestionnaireValidationOutcome: LpaQuestionnaireValidationOutcome | None
    """
    The outcome of the validation action
    """
    lpaQuestionnaireValidationOutcomeDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date the LPA response was validated
    """
    lpaQuestionnaireValidationDetails: list[str] | None
    """
    A list of reasons why the questionnaire is incomplete
    """
    lpaStatement: str | None = Field(
        ..., examples=["We refused the application because of ..."]
    )
    """
    A statement provided by the LPA
    """
    listOfDocumentsBeforeDecision: str | None = None
    """
    Free text response from LPA listing documents used to drive their decision
    """
    caseWithdrawnDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date the appeal was withdrawn by the appellant
    """
    caseTransferredDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date the appeal was transferred to a new case of a different type
    """
    transferredCaseClosedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date the appeal was closed and the appellant requested to resubmit
    """
    caseDecisionOutcomeDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date of the appeal decision
    """
    caseDecisionPublishedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date the appeal decision was published
    """
    caseDecisionOutcome: CaseDecisionOutcome | None
    """
    The final outcome for the decision
    """
    caseCompletedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date the appeal decision letter
    """
    enforcementNotice: bool | None
    """
    Indicates if an enforcement notice is the reason for the appeal
    """
    applicationReference: str | None = Field(..., examples=["43010/APP/1/290572"])
    """
    The unique identifier of the LPA application
    """
    applicationDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date of the original LPA application
    """
    applicationDecision: ApplicationDecision | None
    """
    The outcome of the original LPA decision
    """
    applicationDecisionDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date of the original LPA decision
    """
    applicationDecisionDueDate: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date that an application decision is due
    """
    caseSubmissionDueDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The statutory deadline for submitting an appeal from the original LPA decision date
    """
    siteAddressLine1: str | None = Field(None, examples=["96 The Avenue"])
    """
    First line of address for the appeal site
    """
    siteAddressLine2: str | None = None
    """
    Second line of address for the appeal site
    """
    siteAddressTown: str | None = Field(None, examples=["Maidstone"])
    """
    Town / City of the site address
    """
    siteAddressCounty: str | None = Field(None, examples=["Kent"])
    """
    County of the site address
    """
    siteAddressPostcode: str | None = Field(None, examples=["MD21 5XY"])
    """
    Postal code of the site address
    """
    siteAccessDetails: list[str] | None
    """
    Provided information on site accessibility
    """
    siteSafetyDetails: list[str] | None
    """
    Provided information on site health and safety
    """
    siteAreaSquareMetres: float | None = Field(..., examples=[45.7])
    """
    The site area, in square meters
    """
    floorSpaceSquareMetres: float | None = Field(..., examples=[3.5])
    """
    The floor space, in square meters
    """
    isCorrectAppealType: bool | None
    """
    Indicates if the LPA considers the appeal type appropriate
    """
    isGreenBelt: bool | None
    """
    Indicates if the site is in a green belt
    """
    inConservationArea: bool | None
    """
    Indicates if the site is in a conservation area
    """
    ownsAllLand: bool | None
    """
    Indicates if the appellant has complete ownership of the site
    """
    ownsSomeLand: bool | None
    """
    Indicates if the appellant has partial ownership of the site
    """
    knowsOtherOwners: KnowsOtherOwners | None
    """
    Indicates if the appellant knows other owners of the site
    """
    knowsAllOwners: KnowsAllOwners | None
    """
    Indicates if the appellant knows all owners of the site
    """
    advertisedAppeal: bool | None
    """
    Indicates if the appellant has advertised the appeal to the LPA decision
    """
    notificationMethod: list[NotificationMethodEnum] | None = Field(
        ..., examples=[["notice", "letter"], ["advert"]]
    )
    """
    The methods used to notify relevant parties
    """
    ownersInformed: bool | None
    """
    Indicates if the appellant has informed other owners of the site
    """
    originalDevelopmentDescription: str | None = Field(
        ..., examples=["Replacement of an existing gate to access the property"]
    )
    """
    The original description of the development, as provided by the appellant
    """
    changedDevelopmentDescription: bool | None
    """
    Indicates that the LPA has changed the development description
    """
    newConditionDetails: str | None = Field(
        ...,
        examples=["Since the original application, a number of things have changed..."],
    )
    """
    New conditions details provided by the LPA
    """
    extraConditionsDetails: str | None = Field(
        None,
        examples=["Since the original application, a number of things have changed..."],
    )
    """
    extra conditions details provided by the LPA
    """
    nearbyCaseReferences: list[str] | None
    """
    A list of related case references known to the appellant and the LPA
    """
    neighbouringSiteAddresses: list[NeighbouringSiteAddress] | None
    """
    A list of neighbouring site addresses
    """
    reasonForNeighbourVisits: str | None
    """
    A general reason given for the need to visit any neighbours
    """
    affectedListedBuildingNumbers: list[str] | None
    """
    A list of affected listed building IDs from Historic England
    """
    changedListedBuildingNumbers: list[str] | None
    """
    A list of changed listed building IDs from Historic England
    """
    preserveGrantLoan: bool | None = None
    """
    A grant or loan has been made to preserve the listed building
    """
    consultHistoricEngland: bool | None = None
    """
    Are Historic England to be consulted?
    """
    appellantCostsAppliedFor: bool | None
    """
    Indicates if the appellant has applied for costs
    """
    lpaCostsAppliedFor: bool | None
    """
    Indicates if the appellant has applied for costs
    """
    agriculturalHolding: bool | None
    """
    Indicates if the site is an agricultural holding
    """
    tenantAgriculturalHolding: bool | None
    """
    Indicates if the site is a tenant agricultural holding
    """
    otherTenantsAgriculturalHolding: bool | None
    """
    Indicates if the site is an agricultural holding with other tenants
    """
    informedTenantsAgriculturalHolding: bool | None
    """
    If the site is an agricultural holding, indicates tenants are informed
    """
    appellantProcedurePreference: AppellantProcedurePreference | None
    """
    The procedure preference indicated by the appellant
    """
    appellantProcedurePreferenceDetails: str | None
    """
    The procedure details preference indicated by the appellant
    """
    appellantProcedurePreferenceDuration: float | None
    """
    The duration of the procedure indicated by the appellant
    """
    appellantProcedurePreferenceWitnessCount: float | None
    """
    The number of witnesses appellant has suggested for inquiries preference
    """
    statusPlanningObligation: str | None
    """
    The planning obligation information, if available
    """
    affectsScheduledMonument: bool | None
    """
    Indicates that a scheduled monument is affected
    """
    hasProtectedSpecies: bool | None
    """
    Indicates the existence of protected species
    """
    isAonbNationalLandscape: bool | None
    """
    Indicates an area of outstanding beauty (National Landscape)
    """
    designatedSitesNames: list[str] | None
    """
    The designated site names
    """
    isGypsyOrTravellerSite: bool | None
    """
    Indicates a traveller site
    """
    isPublicRightOfWay: bool | None
    """
    Indicates a PROW (public right of way)
    """
    eiaEnvironmentalImpactSchedule: EiaEnvironmentalImpactSchedule | None
    """
    The impact schedule from EIA
    """
    eiaDevelopmentDescription: EiaDevelopmentDescription | None
    """
    The development description from EIA
    """
    eiaSensitiveAreaDetails: str | None
    """
    The sensitive area details from EIA
    """
    eiaColumnTwoThreshold: bool | None
    """
    EIA specific threshold
    """
    eiaScreeningOpinion: bool | None
    """
    EIA screening opinion
    """
    eiaRequiresEnvironmentalStatement: bool | None
    """
    EIA environmental statement required
    """
    eiaCompletedEnvironmentalStatement: bool | None
    """
    EIA completed environmental statement required
    """
    hasStatutoryConsultees: bool | None
    """
    Indicates statutory consultees
    """
    consultedBodiesDetails: str | None
    """
    The details of the bodies consulted
    """
    hasInfrastructureLevy: bool | None
    """
    Indicates the existence of an infrastructure levy
    """
    isInfrastructureLevyFormallyAdopted: bool | None
    """
    Indicates if the infrastructure levy is formally adopted
    """
    infrastructureLevyAdoptedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date of the infrastructure levy adoption
    """
    infrastructureLevyExpectedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The expected date of the infrastructure levy
    """
    lpaProcedurePreference: LpaProcedurePreference | None
    """
    The procedure preference indicated by the LPA
    """
    lpaProcedurePreferenceDetails: str | None
    """
    The procedure details preference indicated by the LPA
    """
    lpaProcedurePreferenceDuration: float | None
    """
    The duration of enquiry indicated by the LPA
    """
    caseworkReason: str | None
    importantInformation: str | None
    jurisdiction: str | None
    redeterminedIndicator: str | None
    dateCostsReportDespatched: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date of the cost report
    """
    dateNotRecoveredOrDerecovered: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    dateRecovered: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    originalCaseDecisionDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    targetDate: AwareDatetime | None = Field(..., examples=["2023-07-27T20:30:00.000Z"])
    appellantCommentsSubmittedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    appellantStatementDueDate: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    appellantStatementSubmittedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    appellantProofsSubmittedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    finalCommentsDueDate: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    interestedPartyRepsDueDate: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    lpaCommentsSubmittedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    lpaProofsSubmittedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    lpaStatementSubmittedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    proofsOfEvidenceDueDate: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    siteNoticesSentDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    statementDueDate: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    caseManagementConferenceDate: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date of the Case Management Conference (CMC)
    """
    numberOfResidencesNetChange: float | None
    """
    The net gain in residences of the proposed development
    """
    siteGridReferenceEasting: constr(pattern=r"^\d{6,7}$") = Field(
        ..., examples=["357144"]
    )
    siteGridReferenceNorthing: constr(pattern=r"^\d{6,7}$") = Field(
        ..., examples=["400534"]
    )
    siteViewableFromRoad: bool | None
    siteWithinSSSI: bool | None
    typeOfPlanningApplication: TypeOfPlanningApplication | None
    developmentType: DevelopmentType | None
    statementOfCommonGroundDueDate: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    planningObligationDueDate: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    hasLandownersPermission: bool | None = None
    """
    Does the appellant have the landowners permission
    """
    wasApplicationRefusedDueToHighwayOrTraffic: bool | None = None
    """
    Did the LPA refuse the application because of highway or traffic public safety?
    """
    didAppellantSubmitCompletePhotosAndPlans: bool | None = None
    """
    Did the appellant submit complete photos and plans?
    """
    isSiteInAreaOfSpecialControlAdverts: bool | None = None
    """
    Is the site in an area of special control of advertisements?
    """
    advertDetails: list[AdvertDetail] | None = None
    """
    A list of advertisements for the appeal
    """
    padsSapId: str | None = None
    """
    padsSapId for PADS inspectors
    """
    enforcementNoticeReference: str | None = Field(None, examples=["myC45e-123"])
    """
    The reference number of the enforcement notice
    """
    descriptionOfAllegedBreach: str | None = Field(
        None, examples=["My breach description"]
    )
    """
    A description of the alleged breach
    """
    dateAppellantContactedPins: AwareDatetime | None = Field(
        None, examples=["2025-12-26T10:00:00.000Z"]
    )
    """
    The date of the appellant contacted PINS prior to submitting an appeal
    """
    ownerOccupancyStatus: str | None = Field(
        None, examples=["Owner", "Tenant", "Mortgage lender"]
    )
    """
    The nature of the appellant's interest in the appeal site
    """
    occupancyConditionsMet: bool | None = None
    """
    Does the appellant meet the criteria in relation to their interest in the site
    """
    enforcementAppealGroundsDetails: list[EnforcementAppealGroundsDetail] | None = None
    """
    A list of grounds for the appeal, supporting facts & start dates. Enforcement cases (C) can have grounds (a) to (g)
    """
    applicationElbAppealGroundsDetails: (
        list[ApplicationElbAppealGroundsDetail] | None
    ) = None
    """
    A list of grounds for the appeal, supporting facts & start dates. Enforcement cases (C) can have grounds (a) to (g)
    """
    applicationMadeAndFeePaid: bool | None = None
    """
    Was an application made and paid for
    """
    noticeRelatesToBuildingEngineeringMiningOther: bool | None = None
    changeOfUseRefuseOrWaste: bool | None = None
    changeOfUseMineralExtraction: bool | None = None
    changeOfUseMineralStorage: bool | None = None
    relatesToErectionOfBuildingOrBuildings: bool | None = None
    relatesToBuildingWithAgriculturalPurpose: bool | None = None
    relatesToBuildingSingleDwellingHouse: bool | None = None
    previousPlanningPermissionGranted: bool | None = None
    """
    Was planning permission granted in relation to a prior application
    """
    issueDateOfEnforcementNotice: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date on which an Enforcement Notice or Discontinuance Notice was issued
    """
    effectiveDateOfEnforcementNotice: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The effective date of an Enforcement Notice or Discontinuance Notice
    """
    didAppellantAppealLpaDecision: bool | None = None
    """
    Did the appellant appeal the decision made
    """
    dateLpaDecisionDue: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    Date on which LPA decision in relation to application is due
    """
    dateLpaDecisionReceived: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    Date on which LPA decision in relation to application was received
    """
    areaOfAllegedBreachInSquareMetres: float | None = Field(None, examples=[3.5])
    doesAllegedBreachCreateFloorSpace: bool | None = None
    floorSpaceCreatedByBreachInSquareMetres: float | None = Field(None, examples=[5.3])
    applicationPartOrWholeDevelopment: ApplicationPartOrWholeDevelopment | None = None
    """
    Did the application relate to all or part of the development
    """
    hasPcnBeenServed: bool | None = None
    isSiteWithin67MOfTrunkRd: bool | None = None
    affectedTrunkRoadName: str | None = None
    """
    Name of trunk road within 67M of site
    """
    isSiteOnCrownLand: bool | None = None
    stopNoticeExists: bool | None = None
    isSiteWithin400MOfMineralInterest: bool | None = None
    isSiteWithin250MOfLandfill: bool | None = None
    hasDevelopmentInvolvedWasteImportation: bool | None = None
    isSiteSubjectToArticle4Direction: bool | None = None
    article4AffectedDevelopmentRights: str | None = None
    """
    What permitted development rights are affected by the Article 4 Direction
    """
    pcnRestrictedDevelopmentRights: bool | None = None
    """
    Have any development rights been restricted by means of a planning condition?
    """
    applicationMadeUnderActSection: ApplicationMadeUnderActSection | None = None
    siteUseAtTimeOfApplication: str | None = None
    """
    Site use at time of application
    """
    appealUnderActSection: AppealUnderActSection | None = None
    lpaConsiderAppealInvalid: bool | None = None
    lpaAppealInvalidReasons: str | None = None
    """
    Reason(s) why LPA consider the appeal invalid
    """
    discontinuanceNoticeServedReason: DiscontinuanceNoticeServedReason | None = None
    discontinuanceNoticeServedOtherReason: str | None = None
    """
    Other reason for serving a discontinuance notice, for discontinuanceNoticeServedReason: other
    """
    wasRetrospectivePlanningApplicationMade: bool | None = None
    reasonForAppealAppellant: str | None = None
    """
    Reason for appeal as provided by the appellant (Expedited Appeals)
    """
    screeningOpinionIndicatesEiaRequired: bool | None = None
    """
    Indicates if the screening opinion specifies that an EIA is required (Expedited Appeals)
    """
    significantChangesAffectingApplicationAppellant: (
        list[SignificantChangesAffectingApplicationAppellantItem] | None
    ) = None
    """
    Significant changes affecting the application as reported by the appellant (Expedited Appeals)
    """
    significantChangesAffectingApplicationLpa: (
        list[SignificantChangesAffectingApplicationLpaItem] | None
    ) = None
    """
    Significant changes affecting the application as reported by the LPA (Expedited Appeals)
    """


class AdvertDetail1(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    advertType: AdvertType | None
    isAdvertInPosition: bool | None
    """
    Is the advertisement in position?
    """
    isSiteOnHighwayLand: bool | None
    """
    Is the site on highway land?
    """


class AppealGroundLetter2(Enum):
    a = "a"
    b = "b"
    c = "c"
    d = "d"
    e = "e"
    f = "f"
    g = "g"
    NoneType_None = None


class EnforcementAppealGroundsDetail1(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    appealGroundLetter: AppealGroundLetter2 | None = None
    groundForAppealStartDate: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    groundFacts: str | None = None
    """
    Facts supporting the appeal ground
    """


class AppealGroundLetter3(Enum):
    a = "a"
    b = "b"
    c = "c"
    d = "d"
    e = "e"
    f = "f"
    g = "g"
    h = "h"
    i = "i"
    j = "j"
    k = "k"
    NoneType_None = None


class ApplicationElbAppealGroundsDetail1(BaseModel):
    model_config = ConfigDict(
        extra="allow",
    )
    appealGroundLetter: AppealGroundLetter3 | None = None
    groundForAppealStartDate: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    groundFacts: str | None = None
    """
    Facts supporting the appeal ground
    """


class SignificantChangesAffectingApplicationAppellantItem1(BaseModel):
    value: Value | None
    comment: str | None = None


class SignificantChangesAffectingApplicationLpaItem1(BaseModel):
    value: Value | None
    comment: str | None = None


class AppealS78Case2(BaseModel):
    """
    Group B - Schema defining the metadata for an appeal
    """

    model_config = ConfigDict(
        extra="allow",
    )
    caseId: int | None = Field(..., examples=[1200])
    """
    Internal case identifier
    """
    caseReference: str = Field(..., examples=["7002471"])
    """
    External case identifier
    """
    submissionId: UUID | None = Field(
        None, examples=["7f45748c-2342-4b59-b934-ad6183e8d834"]
    )
    """
    Case submission id - link to draft submission created by appellant
    """
    caseStatus: CaseStatus
    """
    The processing status for the appeal
    """
    caseType: CaseType
    """
    The internal code for an appeal type, e.g. D (Householder)
    """
    caseProcedure: CaseProcedure | None
    """
    The type of procedure for the appeal
    """
    lpaCode: str = Field(..., examples=["Q9999"])
    """
    A unique identifier for the Local Planning Authority
    """
    caseOfficerId: str | None
    """
    Unique identifier for the case officer assigned to the case
    """
    inspectorId: str | None
    """
    Unique identifier for the inspector assigned to the case
    """
    allocationLevel: AllocationLevel | None
    """
    A level used for allocation purposes
    """
    allocationBand: AllocationBand | None
    """
    A band used for allocation purposes
    """
    caseSpecialisms: list[str] | None = Field(
        ..., examples=[["Access", "Listed building and enforcement"], ["Schedule 1"]]
    )
    """
    A list of specialisms for allocation purposes
    """
    caseSubmittedDate: AwareDatetime = Field(..., examples=["2023-07-27T20:30:00.000Z"])
    """
    The date the appeal was submitted by the appellant
    """
    caseCreatedDate: AwareDatetime = Field(..., examples=["2023-07-27T20:30:00.000Z"])
    """
    The date the appeal was received
    """
    caseUpdatedDate: AwareDatetime = Field(..., examples=["2023-07-27T20:30:00.000Z"])
    """
    The date the appeal was last updated in the back-office
    """
    caseValidDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date since when the appeal was considered valid
    """
    caseValidationDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date the appeal was validated in the back-office
    """
    caseValidationOutcome: CaseValidationOutcome | None
    """
    The outcome of the validation action
    """
    caseValidationInvalidDetails: list[str] | None
    """
    A list of reasons why the appeal is invalid
    """
    caseValidationIncompleteDetails: list[str] | None
    """
    A list of reasons why the appeal is incomplete
    """
    caseExtensionDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    When the validation outcome is incomplete, an extension may be granted to provide missing information
    """
    caseStartedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    A date indicating when the case was started, resulting in the creation of a timetable
    """
    casePublishedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    A date indicating when the case was published
    """
    linkedCaseStatus: LinkedCaseStatus | None
    """
    Indicates if the case is linked, and the type of relationship
    """
    leadCaseReference: str | None = Field(..., examples=["7002471"])
    """
    The reference of the lead case, if the case is a child case
    """
    lpaQuestionnaireDueDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    If the case is started and has a timetable, a deadline for the LPA to provide a response
    """
    lpaQuestionnaireSubmittedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date the LPA provided a response to the case
    """
    lpaQuestionnaireCreatedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date the LPA response was receeived
    """
    lpaQuestionnairePublishedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date indicating when the questionnaire review was completed and the questionnaire published
    """
    lpaQuestionnaireValidationOutcome: LpaQuestionnaireValidationOutcome | None
    """
    The outcome of the validation action
    """
    lpaQuestionnaireValidationOutcomeDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date the LPA response was validated
    """
    lpaQuestionnaireValidationDetails: list[str] | None
    """
    A list of reasons why the questionnaire is incomplete
    """
    lpaStatement: str | None = Field(
        ..., examples=["We refused the application because of ..."]
    )
    """
    A statement provided by the LPA
    """
    listOfDocumentsBeforeDecision: str | None = None
    """
    Free text response from LPA listing documents used to drive their decision
    """
    caseWithdrawnDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date the appeal was withdrawn by the appellant
    """
    caseTransferredDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date the appeal was transferred to a new case of a different type
    """
    transferredCaseClosedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date the appeal was closed and the appellant requested to resubmit
    """
    caseDecisionOutcomeDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date of the appeal decision
    """
    caseDecisionPublishedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date the appeal decision was published
    """
    caseDecisionOutcome: CaseDecisionOutcome | None
    """
    The final outcome for the decision
    """
    caseCompletedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date the appeal decision letter
    """
    enforcementNotice: bool | None
    """
    Indicates if an enforcement notice is the reason for the appeal
    """
    applicationReference: str | None = Field(..., examples=["43010/APP/1/290572"])
    """
    The unique identifier of the LPA application
    """
    applicationDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date of the original LPA application
    """
    applicationDecision: ApplicationDecision | None
    """
    The outcome of the original LPA decision
    """
    applicationDecisionDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date of the original LPA decision
    """
    applicationDecisionDueDate: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date that an application decision is due
    """
    caseSubmissionDueDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The statutory deadline for submitting an appeal from the original LPA decision date
    """
    siteAddressLine1: str = Field(..., examples=["96 The Avenue"])
    """
    First line of address for the appeal site
    """
    siteAddressLine2: str | None = None
    """
    Second line of address for the appeal site
    """
    siteAddressTown: str = Field(..., examples=["Maidstone"])
    """
    Town / City of the site address
    """
    siteAddressCounty: str | None = Field(None, examples=["Kent"])
    """
    County of the site address
    """
    siteAddressPostcode: str = Field(..., examples=["MD21 5XY"])
    """
    Postal code of the site address
    """
    siteAccessDetails: list[str] | None
    """
    Provided information on site accessibility
    """
    siteSafetyDetails: list[str] | None
    """
    Provided information on site health and safety
    """
    siteAreaSquareMetres: float | None = Field(..., examples=[45.7])
    """
    The site area, in square meters
    """
    floorSpaceSquareMetres: float | None = Field(..., examples=[3.5])
    """
    The floor space, in square meters
    """
    isCorrectAppealType: bool | None
    """
    Indicates if the LPA considers the appeal type appropriate
    """
    isGreenBelt: bool | None
    """
    Indicates if the site is in a green belt
    """
    inConservationArea: bool | None
    """
    Indicates if the site is in a conservation area
    """
    ownsAllLand: bool | None
    """
    Indicates if the appellant has complete ownership of the site
    """
    ownsSomeLand: bool | None
    """
    Indicates if the appellant has partial ownership of the site
    """
    knowsOtherOwners: KnowsOtherOwners | None
    """
    Indicates if the appellant knows other owners of the site
    """
    knowsAllOwners: KnowsAllOwners | None
    """
    Indicates if the appellant knows all owners of the site
    """
    advertisedAppeal: bool | None
    """
    Indicates if the appellant has advertised the appeal to the LPA decision
    """
    notificationMethod: list[NotificationMethodEnum] | None = Field(
        ..., examples=[["notice", "letter"], ["advert"]]
    )
    """
    The methods used to notify relevant parties
    """
    ownersInformed: bool | None
    """
    Indicates if the appellant has informed other owners of the site
    """
    originalDevelopmentDescription: str | None = Field(
        ..., examples=["Replacement of an existing gate to access the property"]
    )
    """
    The original description of the development, as provided by the appellant
    """
    changedDevelopmentDescription: bool | None
    """
    Indicates that the LPA has changed the development description
    """
    newConditionDetails: str | None = Field(
        ...,
        examples=["Since the original application, a number of things have changed..."],
    )
    """
    New conditions details provided by the LPA
    """
    extraConditionsDetails: str | None = Field(
        None,
        examples=["Since the original application, a number of things have changed..."],
    )
    """
    extra conditions details provided by the LPA
    """
    nearbyCaseReferences: list[str] | None
    """
    A list of related case references known to the appellant and the LPA
    """
    neighbouringSiteAddresses: list[NeighbouringSiteAddress] | None
    """
    A list of neighbouring site addresses
    """
    reasonForNeighbourVisits: str | None
    """
    A general reason given for the need to visit any neighbours
    """
    affectedListedBuildingNumbers: list[str] | None
    """
    A list of affected listed building IDs from Historic England
    """
    changedListedBuildingNumbers: list[str] | None
    """
    A list of changed listed building IDs from Historic England
    """
    preserveGrantLoan: bool | None = None
    """
    A grant or loan has been made to preserve the listed building
    """
    consultHistoricEngland: bool | None = None
    """
    Are Historic England to be consulted?
    """
    appellantCostsAppliedFor: bool | None
    """
    Indicates if the appellant has applied for costs
    """
    lpaCostsAppliedFor: bool | None
    """
    Indicates if the appellant has applied for costs
    """
    agriculturalHolding: bool | None
    """
    Indicates if the site is an agricultural holding
    """
    tenantAgriculturalHolding: bool | None
    """
    Indicates if the site is a tenant agricultural holding
    """
    otherTenantsAgriculturalHolding: bool | None
    """
    Indicates if the site is an agricultural holding with other tenants
    """
    informedTenantsAgriculturalHolding: bool | None
    """
    If the site is an agricultural holding, indicates tenants are informed
    """
    appellantProcedurePreference: AppellantProcedurePreference | None
    """
    The procedure preference indicated by the appellant
    """
    appellantProcedurePreferenceDetails: str | None
    """
    The procedure details preference indicated by the appellant
    """
    appellantProcedurePreferenceDuration: float | None
    """
    The duration of the procedure indicated by the appellant
    """
    appellantProcedurePreferenceWitnessCount: float | None
    """
    The number of witnesses appellant has suggested for inquiries preference
    """
    statusPlanningObligation: str | None
    """
    The planning obligation information, if available
    """
    affectsScheduledMonument: bool | None
    """
    Indicates that a scheduled monument is affected
    """
    hasProtectedSpecies: bool | None
    """
    Indicates the existence of protected species
    """
    isAonbNationalLandscape: bool | None
    """
    Indicates an area of outstanding beauty (National Landscape)
    """
    designatedSitesNames: list[str] | None
    """
    The designated site names
    """
    isGypsyOrTravellerSite: bool | None
    """
    Indicates a traveller site
    """
    isPublicRightOfWay: bool | None
    """
    Indicates a PROW (public right of way)
    """
    eiaEnvironmentalImpactSchedule: EiaEnvironmentalImpactSchedule | None
    """
    The impact schedule from EIA
    """
    eiaDevelopmentDescription: EiaDevelopmentDescription | None
    """
    The development description from EIA
    """
    eiaSensitiveAreaDetails: str | None
    """
    The sensitive area details from EIA
    """
    eiaColumnTwoThreshold: bool | None
    """
    EIA specific threshold
    """
    eiaScreeningOpinion: bool | None
    """
    EIA screening opinion
    """
    eiaRequiresEnvironmentalStatement: bool | None
    """
    EIA environmental statement required
    """
    eiaCompletedEnvironmentalStatement: bool | None
    """
    EIA completed environmental statement required
    """
    hasStatutoryConsultees: bool | None
    """
    Indicates statutory consultees
    """
    consultedBodiesDetails: str | None
    """
    The details of the bodies consulted
    """
    hasInfrastructureLevy: bool | None
    """
    Indicates the existence of an infrastructure levy
    """
    isInfrastructureLevyFormallyAdopted: bool | None
    """
    Indicates if the infrastructure levy is formally adopted
    """
    infrastructureLevyAdoptedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date of the infrastructure levy adoption
    """
    infrastructureLevyExpectedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The expected date of the infrastructure levy
    """
    lpaProcedurePreference: LpaProcedurePreference | None
    """
    The procedure preference indicated by the LPA
    """
    lpaProcedurePreferenceDetails: str | None
    """
    The procedure details preference indicated by the LPA
    """
    lpaProcedurePreferenceDuration: float | None
    """
    The duration of enquiry indicated by the LPA
    """
    caseworkReason: str | None
    importantInformation: str | None
    jurisdiction: str | None
    redeterminedIndicator: str | None
    dateCostsReportDespatched: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date of the cost report
    """
    dateNotRecoveredOrDerecovered: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    dateRecovered: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    originalCaseDecisionDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    targetDate: AwareDatetime | None = Field(..., examples=["2023-07-27T20:30:00.000Z"])
    appellantCommentsSubmittedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    appellantStatementDueDate: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    appellantStatementSubmittedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    appellantProofsSubmittedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    finalCommentsDueDate: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    interestedPartyRepsDueDate: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    lpaCommentsSubmittedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    lpaProofsSubmittedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    lpaStatementSubmittedDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    proofsOfEvidenceDueDate: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    siteNoticesSentDate: AwareDatetime | None = Field(
        ..., examples=["2023-07-27T20:30:00.000Z"]
    )
    statementDueDate: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    caseManagementConferenceDate: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date of the Case Management Conference (CMC)
    """
    numberOfResidencesNetChange: float | None
    """
    The net gain in residences of the proposed development
    """
    siteGridReferenceEasting: constr(pattern=r"^\d{6,7}$") | None = Field(
        None, examples=["357144"]
    )
    siteGridReferenceNorthing: constr(pattern=r"^\d{6,7}$") | None = Field(
        None, examples=["400534"]
    )
    siteViewableFromRoad: bool | None
    siteWithinSSSI: bool | None
    typeOfPlanningApplication: TypeOfPlanningApplication | None
    developmentType: DevelopmentType | None
    statementOfCommonGroundDueDate: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    planningObligationDueDate: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    hasLandownersPermission: bool | None = None
    """
    Does the appellant have the landowners permission
    """
    wasApplicationRefusedDueToHighwayOrTraffic: bool | None = None
    """
    Did the LPA refuse the application because of highway or traffic public safety?
    """
    didAppellantSubmitCompletePhotosAndPlans: bool | None = None
    """
    Did the appellant submit complete photos and plans?
    """
    isSiteInAreaOfSpecialControlAdverts: bool | None = None
    """
    Is the site in an area of special control of advertisements?
    """
    advertDetails: list[AdvertDetail1] | None = None
    """
    A list of advertisements for the appeal
    """
    padsSapId: str | None = None
    """
    padsSapId for PADS inspectors
    """
    enforcementNoticeReference: str | None = Field(None, examples=["myC45e-123"])
    """
    The reference number of the enforcement notice
    """
    descriptionOfAllegedBreach: str | None = Field(
        None, examples=["My breach description"]
    )
    """
    A description of the alleged breach
    """
    dateAppellantContactedPins: AwareDatetime | None = Field(
        None, examples=["2025-12-26T10:00:00.000Z"]
    )
    """
    The date of the appellant contacted PINS prior to submitting an appeal
    """
    ownerOccupancyStatus: str | None = Field(
        None, examples=["Owner", "Tenant", "Mortgage lender"]
    )
    """
    The nature of the appellant's interest in the appeal site
    """
    occupancyConditionsMet: bool | None = None
    """
    Does the appellant meet the criteria in relation to their interest in the site
    """
    enforcementAppealGroundsDetails: list[EnforcementAppealGroundsDetail1] | None = None
    """
    A list of grounds for the appeal, supporting facts & start dates. Enforcement cases (C) can have grounds (a) to (g)
    """
    applicationElbAppealGroundsDetails: (
        list[ApplicationElbAppealGroundsDetail1] | None
    ) = None
    """
    A list of grounds for the appeal, supporting facts & start dates. Enforcement cases (C) can have grounds (a) to (g)
    """
    applicationMadeAndFeePaid: bool | None = None
    """
    Was an application made and paid for
    """
    noticeRelatesToBuildingEngineeringMiningOther: bool | None = None
    changeOfUseRefuseOrWaste: bool | None = None
    changeOfUseMineralExtraction: bool | None = None
    changeOfUseMineralStorage: bool | None = None
    relatesToErectionOfBuildingOrBuildings: bool | None = None
    relatesToBuildingWithAgriculturalPurpose: bool | None = None
    relatesToBuildingSingleDwellingHouse: bool | None = None
    previousPlanningPermissionGranted: bool | None = None
    """
    Was planning permission granted in relation to a prior application
    """
    issueDateOfEnforcementNotice: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The date on which an Enforcement Notice or Discontinuance Notice was issued
    """
    effectiveDateOfEnforcementNotice: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    The effective date of an Enforcement Notice or Discontinuance Notice
    """
    didAppellantAppealLpaDecision: bool | None = None
    """
    Did the appellant appeal the decision made
    """
    dateLpaDecisionDue: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    Date on which LPA decision in relation to application is due
    """
    dateLpaDecisionReceived: AwareDatetime | None = Field(
        None, examples=["2023-07-27T20:30:00.000Z"]
    )
    """
    Date on which LPA decision in relation to application was received
    """
    areaOfAllegedBreachInSquareMetres: float | None = Field(None, examples=[3.5])
    doesAllegedBreachCreateFloorSpace: bool | None = None
    floorSpaceCreatedByBreachInSquareMetres: float | None = Field(None, examples=[5.3])
    applicationPartOrWholeDevelopment: ApplicationPartOrWholeDevelopment | None = None
    """
    Did the application relate to all or part of the development
    """
    hasPcnBeenServed: bool | None = None
    isSiteWithin67MOfTrunkRd: bool | None = None
    affectedTrunkRoadName: str | None = None
    """
    Name of trunk road within 67M of site
    """
    isSiteOnCrownLand: bool | None = None
    stopNoticeExists: bool | None = None
    isSiteWithin400MOfMineralInterest: bool | None = None
    isSiteWithin250MOfLandfill: bool | None = None
    hasDevelopmentInvolvedWasteImportation: bool | None = None
    isSiteSubjectToArticle4Direction: bool | None = None
    article4AffectedDevelopmentRights: str | None = None
    """
    What permitted development rights are affected by the Article 4 Direction
    """
    pcnRestrictedDevelopmentRights: bool | None = None
    """
    Have any development rights been restricted by means of a planning condition?
    """
    applicationMadeUnderActSection: ApplicationMadeUnderActSection | None = None
    siteUseAtTimeOfApplication: str | None = None
    """
    Site use at time of application
    """
    appealUnderActSection: AppealUnderActSection | None = None
    lpaConsiderAppealInvalid: bool | None = None
    lpaAppealInvalidReasons: str | None = None
    """
    Reason(s) why LPA consider the appeal invalid
    """
    discontinuanceNoticeServedReason: DiscontinuanceNoticeServedReason | None = None
    discontinuanceNoticeServedOtherReason: str | None = None
    """
    Other reason for serving a discontinuance notice, for discontinuanceNoticeServedReason: other
    """
    wasRetrospectivePlanningApplicationMade: bool | None = None
    reasonForAppealAppellant: str | None = None
    """
    Reason for appeal as provided by the appellant (Expedited Appeals)
    """
    screeningOpinionIndicatesEiaRequired: bool | None = None
    """
    Indicates if the screening opinion specifies that an EIA is required (Expedited Appeals)
    """
    significantChangesAffectingApplicationAppellant: (
        list[SignificantChangesAffectingApplicationAppellantItem1] | None
    ) = None
    """
    Significant changes affecting the application as reported by the appellant (Expedited Appeals)
    """
    significantChangesAffectingApplicationLpa: (
        list[SignificantChangesAffectingApplicationLpaItem1] | None
    ) = None
    """
    Significant changes affecting the application as reported by the LPA (Expedited Appeals)
    """


class AppealS78Case(RootModel[AppealS78Case1 | AppealS78Case2]):
    root: AppealS78Case1 | AppealS78Case2 = Field(..., title="Appeal S78 Case")
    """
    Group B - Schema defining the metadata for an appeal
    """


class SignificantChangesAffectingApplication(Enum):
    adopted_a_new_local_plan = "adopted-a-new-local-plan"
    national_policy_change = "national-policy-change"
    court_judgement = "court-judgement"
    other = "other"
    NoneType_None = None
