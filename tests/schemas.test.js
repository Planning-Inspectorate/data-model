...
describe('schemas', () => {
	it('should only include valid JSON schemas', () => {
		...
		if (Object.keys(errorsBySchema).length) {
			...
			throw new Error(
				`${Object.keys(errorsBySchema).join(', ')} schemas are invalid (count: ${Object.keys(errorsBySchema).length}), see console output`
			);
		}
	});
	it('should have matching $id and filenames', () => {
		...
		if (Object.keys(errorsBySchema).length) {
			...
			throw new Error(
				`${Object.keys(errorsBySchema).join(', ')} schemas have mismatched $id and filenames (count: ${Object.keys(errorsBySchema).length}), see console output`
			);
		}
	});
