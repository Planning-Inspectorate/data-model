...
const exampleEnforcementQuestionnaireSchema = {
	casedata: {
		caseType: 'C',
...
describe('Enforcement questionnaire command schema', () => {
	...
	it('should allow valid schema for Enforcement', () => {
		const validationResult = ajv.validate(schema, exampleEnforcementQuestionnaireSchema);
		assert.strictEqual(validationResult, true);
	});

	it('should reject missing caseReference', () => {
		const test = structuredClone(exampleEnforcementQuestionnaireSchema);
